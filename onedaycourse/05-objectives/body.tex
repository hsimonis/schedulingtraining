...
\begin{itemize}
\item For most scheduling problem, we define some form of objective
...
\item Prefer all machine to end early
...
\item In the example problem, many due dates far in the future
...
\begin{frame}
\frametitle{Including Optional Work \navail}
\begin{itemize}
\item So far, the set of orders had to be fulfilled
\item The resulting jobs needed to be included in schedule
\item Sometimes, there are optional orders that we may or may not include in schedule
\item The more work we do in this schedule, the better
\begin{itemize}
\item Assumes a fixed horizon to limit the available space
\end{itemize}
\item But we can reject some of the orders
\item The objective is to maximize the value of the accepted orders
\item Related to \emph{Knapsack problem}, a well known combinatorial problem
\end{itemize}
\end{frame}
...
\item We need to find all best compromises between the different objective types
\begin{itemize}
...
\item Give effective workers always more work
