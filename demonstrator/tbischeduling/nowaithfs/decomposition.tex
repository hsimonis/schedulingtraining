...
Each stage has the same number of identical machines $m_{s}$.
...
Alternatively, we can state that all tasks of stage $i$ must satisfy a \texttt{diffn} constraint
\begin{equation}
\textrm{diffn}([s_{i1},s_{i2},...,s_{in}],[m_{i1}, m_{i2},...,m_{in}],[1,1,...,1],[1,1,...,1])
\end{equation}
The constraint states that the unit (length and height equal one) rectangles formed by the tasks do not overlap.
...
The following constraint links the ordering on the machines with the start times. If two jobs $j_{1}$ and $j_{2}$ in stage $i$ are scheduled on the same machine, then their start times must follow the ordering of the jobs on the machine:
...
We have stated in constraint~(\ref{eq:order}) that if one job follows another in some stage $i$, then it must also follow that job in the next stage, unless it is assigned to another machine. 
...
At any stage, we can create a feasible schedule by repeating the last job order obtained for stage $i$ for all remaining stages $i+1$ to $s$. This is always possible, but may required to extend the distances between jobs in the first stage to avoid overlap in one of the later stages. 
...
Consider the sequence of jobs traversed by an optimal solution of the TSP, we name it $[j_{1}, j_{2},...,j_{n-1},j_{n}]$. If we schedule these jobs in order, 
...
We may be able to use the cost of the no-wait flow-shop to create a lower bound on the no-wait hybrid flow-shop problem with multiple, identical machines. If every stage has $p$ identical machines, we would expect that there is no solution better than $C_{\textrm{TSP}}/p$.

We can aalso partition the problem into $p$ independent sub-problems by partitioning the jobs into $p$ groups, and then solve each sub-problem to optimality. This would create an upper bound on the overall problem. We can also create a problem with $p$ auxiliary nodes and find $p$ cycles in the graph, forcing exactly one auxiliary node in each cycle. This would handle the best allocation of jobs to groups within the optimization routine. This probably cannot be expressed directly in Concorde, but would be possible using the \texttt{cycle} constraint in CHIP.
