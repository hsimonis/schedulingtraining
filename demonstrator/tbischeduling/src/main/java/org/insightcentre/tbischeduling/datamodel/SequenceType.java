...
public static SequenceType[] cachedValues(){
    if (cache== null){
        cache = SequenceType.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[SequenceType.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(SequenceType p:SequenceType.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[SequenceType.cachedValues().length];
    int i=0;
    for(SequenceType p:SequenceType.cachedValues()){
        res[i++] = p.name();
    }
...
public static SequenceType entry(int i){
...
public static int indexOf(SequenceType p){
    return p.ordinal();
}
