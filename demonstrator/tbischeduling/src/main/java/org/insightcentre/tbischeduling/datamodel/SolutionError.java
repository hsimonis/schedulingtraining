...
public  class SolutionError extends ApplicationObject{
/**
 *  
 *
*/
...
*/
    ...
    public SolutionError(ApplicationDataset applicationDataset){
        ...
        setSolution(null);
        setValue("");
        applicationDataset.addSolutionError(this);
    }
...
*/
    ...
    public SolutionError(SolutionError other){
...
*/
    ...
    public Boolean remove(){
        return getApplicationDataset().removeSolutionError(this) && getApplicationDataset().removeApplicationObject(this);
    }
...
*/
    ...
    public String prettyString(){
        return ""+ " " +getId()+ " " +getName()+ " " +getClassDesc()+ " " +getDescription()+ " " +getField()+ " " +getItem()+ " " +getSeverity()+ " " +getSolution().toColumnString()+ " " +getValue();
    }
...
*/
     ...
     public void toXML(PrintWriter out){
         out.println("<solutionError "+ " applicationDataset=\""+toXMLApplicationDataset()+"\""+
            " id=\""+toXMLId()+"\""+
            " name=\""+toXMLName()+"\""+
...
*/
    ...
    public static String toHTMLLabels(){
        return "<tr><th>SolutionError</th>"+"<th>Name</th>"+"<th>Solution</th>"+"<th>ClassDesc</th>"+"<th>Item</th>"+"<th>Field</th>"+"<th>Value</th>"+"<th>Description</th>"+"<th>Severity</th>"+"</tr>";
    }

    public String toHTML(){
        return "<tr><th>&nbsp;</th>"+"<td>"+getName()+"</td>"+ " " +"<td>"+getSolution().toColumnString()+"</td>"+ " " +"<td>"+getClassDesc()+"</td>"+ " " +"<td>"+getItem()+"</td>"+ " " +"<td>"+getField()+"</td>"+ " " +"<td>"+getValue()+"</td>"+ " " +"<td>"+getDescription()+"</td>"+ " " +"<td>"+getSeverity()+"</td>"+"</tr>";
    }

/**
 * find the same object in another dataset
 * @param a SolutionError item we are looking for
 * @param bList List<SolutionError> list of items in which we are searching
 * @return SolutionError entry of list b which is applicationSame() to a
*/

    public static SolutionError find(SolutionError a, List<SolutionError> bList){
 ...
 * @param name SolutionError name of the object we are looking for
 * @return SolutionError entry of the dataset with the given name; otherwise null
*/

    public static SolutionError findByName(ApplicationDataset base, String name){
        for(SolutionError a:base.getListSolutionError()) {
 ...
 * @param base  dataset in which we are searching
 * @param name SolutionError name of the object we are looking for
 * @return SolutionError entry of the dataset with the given name
*/

    public static SolutionError findOrCreate(ApplicationDataset base, String name){
        ...
        SolutionError res = new SolutionError(base);
        res.setName(name);
        return res;
    }
 ...
 * @param base dataset in which we are searching
 * @return SolutionError first entry in the dataset of this type; null if that does not exists
*/

    public static SolutionError findFirst(ApplicationDataset base){
        if (base.getListSolutionError().isEmpty()) {
            return null;
        }
        return base.getListSolutionError().get(0);
    }

/**
 * find some entry entry in the dataset of that type
 * @param base dataset in which we are searching
 * @return SolutionError some entry in the dataset of this type; null if that does not exists
*/

    public static SolutionError findAny(ApplicationDataset base){
        int size=base.getListSolutionError().size();
        if (size > 0) {
             return base.getListSolutionError().get(new Random().nextInt(size));
        }
        return null;
    }
 ...
 * @param base dataset in which we are searching
 * @return SolutionError last entry in the dataset of this type; null if that does not exists
*/

    public static SolutionError findLast(ApplicationDataset base){
        int size=base.getListSolutionError().size();
        if (size > 0) {
             return base.getListSolutionError().get(size-1);
        }
        return null;
    }
 ...
 * @param b SolutionError compare this to that object
 * @return Boolean true if the objects match the same criteria
*/

    public Boolean applicationSame(SolutionError b){
...
*/
    ...
    public Boolean applicationEqual(SolutionError b){
...
*/
    ...
    public void check(){
        if (getApplicationDataset() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"applicationDataset","SolutionError",(getApplicationDataset()==null?"null":getApplicationDataset().toString()),"",WarningType.NOTNULL);
        }
        if (getSolution() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"solution","SolutionError",(getSolution()==null?"null":getSolution().toString()),"",WarningType.NOTNULL);
        }
    }

    static void dummy(ApplicationDataset base){
// no dummy information for class SolutionError
    }
