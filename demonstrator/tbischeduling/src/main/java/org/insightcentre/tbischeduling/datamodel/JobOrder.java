...
public static JobOrder[] cachedValues(){
    if (cache== null){
        cache = JobOrder.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[JobOrder.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(JobOrder p:JobOrder.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[JobOrder.cachedValues().length];
    int i=0;
    for(JobOrder p:JobOrder.cachedValues()){
        res[i++] = p.name();
    }
...
public static JobOrder entry(int i){
...
public static int indexOf(JobOrder p){
    return p.ordinal();
}
