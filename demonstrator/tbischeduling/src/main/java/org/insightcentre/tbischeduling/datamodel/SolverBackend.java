...
Cplex;
private static SolverBackend[] cache = null;

public static SolverBackend[] cachedValues(){
    if (cache== null){
        cache = SolverBackend.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[SolverBackend.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(SolverBackend p:SolverBackend.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[SolverBackend.cachedValues().length];
    int i=0;
    for(SolverBackend p:SolverBackend.cachedValues()){
        res[i++] = p.name();
    }
...
public static SolverBackend entry(int i){
...
public static int indexOf(SolverBackend p){
    return p.ordinal();
}
