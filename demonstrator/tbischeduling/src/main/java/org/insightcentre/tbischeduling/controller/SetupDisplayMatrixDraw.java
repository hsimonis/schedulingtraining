...
import static org.insightcentre.tbischeduling.logging.LogShortcut.severe;
   ...
   public class SetupDisplayMatrixDraw {
       public SetupDisplayMatrixDraw(Scenario base, String fileName, String title) {
           try {
       ...
       public static String createContent(Scenario base,String title) {
            ...
            for(SetupType row:findRows(base)){
                sb.append(String.format("['%s'",safe(row)));
                for(SetupType col:findColumns(base)){
        ...
        private static List<SetupType> findColumns(Scenario base){
             return base.getListSetupType().stream().sorted(Comparator.comparing(SetupType::getName)).collect(toList());
        }

        private static List<SetupType> findRows(Scenario base){
             return base.getListSetupType().stream().sorted(Comparator.comparing(SetupType::getName)).collect(toList());
        }
        ...
        private static String findCell(Scenario base,SetupType row,SetupType col){
