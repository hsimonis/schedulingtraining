...
None;
private static ColorBy[] cache = null;

public static ColorBy[] cachedValues(){
    if (cache== null){
        cache = ColorBy.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[ColorBy.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(ColorBy p:ColorBy.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[ColorBy.cachedValues().length];
    int i=0;
    for(ColorBy p:ColorBy.cachedValues()){
        res[i++] = p.name();
    }
...
public static ColorBy entry(int i){
...
public static int indexOf(ColorBy p){
    return p.ordinal();
}
