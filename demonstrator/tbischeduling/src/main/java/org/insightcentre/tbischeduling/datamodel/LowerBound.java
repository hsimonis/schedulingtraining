...
public abstract class LowerBound extends ApplicationObject{
/**
 *  
 *
*/
...
*/
    ...
    public LowerBound(ApplicationDataset applicationDataset){
        super(applicationDataset);
        setDescription("");
        setValue(0);
        applicationDataset.addLowerBound(this);
    }
...
*/
    ...
    public LowerBound(LowerBound other){
...
*/
    ...
    public Boolean remove(){
        return getApplicationDataset().removeLowerBound(this) && getApplicationDataset().removeApplicationObject(this);
    }
...
*/
     ...
     public void toXML(PrintWriter out){
         out.println("<lowerBound "+ " applicationDataset=\""+toXMLApplicationDataset()+"\""+
            " id=\""+toXMLId()+"\""+
            " name=\""+toXMLName()+"\""+
 ...
 * @param a LowerBound item we are looking for
 * @param bList List<LowerBound> list of items in which we are searching
 * @return LowerBound entry of list b which is applicationSame() to a
*/

    public static LowerBound find(LowerBound a, List<LowerBound> bList){
        for(LowerBound b : bList){
 ...
 * @param name LowerBound name of the object we are looking for
 * @return LowerBound entry of the dataset with the given name; otherwise null
*/

    public static LowerBound findByName(ApplicationDataset base, String name){
        for(LowerBound a:base.getListLowerBound()) {
 ...
 * @param base dataset in which we are searching
 * @return LowerBound first entry in the dataset of this type; null if that does not exists
*/

    public static LowerBound findFirst(ApplicationDataset base){
        if (base.getListLowerBound().isEmpty()) {
            return null;
        }
        return base.getListLowerBound().get(0);
    }

/**
 * find some entry entry in the dataset of that type
 * @param base dataset in which we are searching
 * @return LowerBound some entry in the dataset of this type; null if that does not exists
*/

    public static LowerBound findAny(ApplicationDataset base){
        int size=base.getListLowerBound().size();
        if (size > 0) {
             return base.getListLowerBound().get(new Random().nextInt(size));
        }
        return null;
    }
 ...
 * @param base dataset in which we are searching
 * @return LowerBound last entry in the dataset of this type; null if that does not exists
*/

    public static LowerBound findLast(ApplicationDataset base){
        int size=base.getListLowerBound().size();
        if (size > 0) {
             return base.getListLowerBound().get(size-1);
        }
        return null;
    }
 ...
 * @param b LowerBound compare this to that object
 * @return Boolean true if the objects match the same criteria
*/

    public Boolean applicationSame(LowerBound b){
...
*/
    ...
    public Boolean applicationEqual(LowerBound b){
...
*/
    ...
    public void check(){
        if (getApplicationDataset() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"applicationDataset","LowerBound",(getApplicationDataset()==null?"null":getApplicationDataset().toString()),"",WarningType.NOTNULL);
        }
    }
