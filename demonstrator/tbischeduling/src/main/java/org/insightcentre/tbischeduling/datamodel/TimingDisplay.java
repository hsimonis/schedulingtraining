...
StartDurationEnd;
private static TimingDisplay[] cache = null;

public static TimingDisplay[] cachedValues(){
    if (cache== null){
        cache = TimingDisplay.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[TimingDisplay.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(TimingDisplay p:TimingDisplay.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[TimingDisplay.cachedValues().length];
    int i=0;
    for(TimingDisplay p:TimingDisplay.cachedValues()){
        res[i++] = p.name();
    }
...
public static TimingDisplay entry(int i){
...
public static int indexOf(TimingDisplay p){
    return p.ordinal();
}
