...
FitAll;
private static ResourceZoom[] cache = null;

public static ResourceZoom[] cachedValues(){
    if (cache== null){
        cache = ResourceZoom.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[ResourceZoom.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(ResourceZoom p:ResourceZoom.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[ResourceZoom.cachedValues().length];
    int i=0;
    for(ResourceZoom p:ResourceZoom.cachedValues()){
        res[i++] = p.name();
    }
...
public static ResourceZoom entry(int i){
...
public static int indexOf(ResourceZoom p){
    return p.ordinal();
}
