...
public static ResourceModel[] cachedValues(){
    if (cache== null){
        cache = ResourceModel.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[ResourceModel.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(ResourceModel p:ResourceModel.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[ResourceModel.cachedValues().length];
    int i=0;
    for(ResourceModel p:ResourceModel.cachedValues()){
        res[i++] = p.name();
    }
...
public static ResourceModel entry(int i){
...
public static int indexOf(ResourceModel p){
    return p.ordinal();
}
