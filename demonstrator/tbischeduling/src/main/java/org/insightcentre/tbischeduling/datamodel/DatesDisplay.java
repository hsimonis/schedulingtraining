...
External;
private static DatesDisplay[] cache = null;

public static DatesDisplay[] cachedValues(){
    if (cache== null){
        cache = DatesDisplay.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[DatesDisplay.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(DatesDisplay p:DatesDisplay.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[DatesDisplay.cachedValues().length];
    int i=0;
    for(DatesDisplay p:DatesDisplay.cachedValues()){
        res[i++] = p.name();
    }
...
public static DatesDisplay entry(int i){
...
public static int indexOf(DatesDisplay p){
    return p.ordinal();
}
