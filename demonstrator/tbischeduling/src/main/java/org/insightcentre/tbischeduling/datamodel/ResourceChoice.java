...
Selected;
private static ResourceChoice[] cache = null;

public static ResourceChoice[] cachedValues(){
    if (cache== null){
        cache = ResourceChoice.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[ResourceChoice.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(ResourceChoice p:ResourceChoice.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[ResourceChoice.cachedValues().length];
    int i=0;
    for(ResourceChoice p:ResourceChoice.cachedValues()){
        res[i++] = p.name();
    }
...
public static ResourceChoice entry(int i){
...
public static int indexOf(ResourceChoice p){
    return p.ordinal();
}
