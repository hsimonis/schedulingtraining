...
import static org.insightcentre.tbischeduling.logging.LogShortcut.severe;
   ...
   public class DisjunctiveNeedsMatrixDraw {
       public DisjunctiveNeedsMatrixDraw(Scenario base, String fileName, String title) {
           try {
       ...
       public static String createContent(Scenario base,String title) {
        ...
        private static void initHash(Scenario base){
            for(ResourceNeed item:base.getListResourceNeed()){
               hash.put(item.getProcessStep().getId()+":"+item.getDisjunctiveResource().getId(),item.getValue().toString());
            }
        }
        private static String findCell(Scenario base,ProcessStep row,DisjunctiveResource col){
