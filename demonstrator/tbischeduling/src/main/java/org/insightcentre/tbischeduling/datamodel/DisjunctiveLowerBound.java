...
public  class DisjunctiveLowerBound extends LowerBound{
/**
 *  
 *
*/
...
*/
    ...
    public DisjunctiveLowerBound(ApplicationDataset applicationDataset){
        super(applicationDataset);
        setDisjunctiveResource(null);
        applicationDataset.addDisjunctiveLowerBound(this);
    }
...
*/
    ...
    public DisjunctiveLowerBound(DisjunctiveLowerBound other){
...
*/
    ...
    public Boolean remove(){
        return getApplicationDataset().removeDisjunctiveLowerBound(this) && getApplicationDataset().removeLowerBound(this) && getApplicationDataset().removeApplicationObject(this);
    }
...
*/
    ...
    public String prettyString(){
        return ""+ " " +getId()+ " " +getName()+ " " +getDescription()+ " " +getValue()+ " " +getDisjunctiveResource().toColumnString();
    }
...
*/
     ...
     public void toXML(PrintWriter out){
         out.println("<disjunctiveLowerBound "+ " applicationDataset=\""+toXMLApplicationDataset()+"\""+
            " id=\""+toXMLId()+"\""+
            " name=\""+toXMLName()+"\""+
...
*/
    ...
    public static String toHTMLLabels(){
        return "<tr><th>DisjunctiveLowerBound</th>"+"<th>Name</th>"+"<th>Value</th>"+"<th>Description</th>"+"<th>DisjunctiveResource</th>"+"</tr>";
    }

    public String toHTML(){
        return "<tr><th>&nbsp;</th>"+"<td>"+getName()+"</td>"+ " " +"<td>"+getValue()+"</td>"+ " " +"<td>"+getDescription()+"</td>"+ " " +"<td>"+getDisjunctiveResource().toColumnString()+"</td>"+"</tr>";
    }

/**
 * find the same object in another dataset
 * @param a DisjunctiveLowerBound item we are looking for
 * @param bList List<DisjunctiveLowerBound> list of items in which we are searching
 * @return DisjunctiveLowerBound entry of list b which is applicationSame() to a
*/

    public static DisjunctiveLowerBound find(DisjunctiveLowerBound a, List<DisjunctiveLowerBound> bList){
 ...
 * @param name DisjunctiveLowerBound name of the object we are looking for
 * @return DisjunctiveLowerBound entry of the dataset with the given name; otherwise null
*/

    public static DisjunctiveLowerBound findByName(ApplicationDataset base, String name){
        for(DisjunctiveLowerBound a:base.getListDisjunctiveLowerBound()) {
 ...
 * @param base  dataset in which we are searching
 * @param name DisjunctiveLowerBound name of the object we are looking for
 * @return DisjunctiveLowerBound entry of the dataset with the given name
*/

    public static DisjunctiveLowerBound findOrCreate(ApplicationDataset base, String name){
        ...
        DisjunctiveLowerBound res = new DisjunctiveLowerBound(base);
        res.setName(name);
        return res;
    }
 ...
 * @param base dataset in which we are searching
 * @return DisjunctiveLowerBound first entry in the dataset of this type; null if that does not exists
*/

    public static DisjunctiveLowerBound findFirst(ApplicationDataset base){
        if (base.getListDisjunctiveLowerBound().isEmpty()) {
            return null;
        }
        return base.getListDisjunctiveLowerBound().get(0);
    }

/**
 * find some entry entry in the dataset of that type
 * @param base dataset in which we are searching
 * @return DisjunctiveLowerBound some entry in the dataset of this type; null if that does not exists
*/

    public static DisjunctiveLowerBound findAny(ApplicationDataset base){
        int size=base.getListDisjunctiveLowerBound().size();
        if (size > 0) {
             return base.getListDisjunctiveLowerBound().get(new Random().nextInt(size));
        }
        return null;
    }
 ...
 * @param base dataset in which we are searching
 * @return DisjunctiveLowerBound last entry in the dataset of this type; null if that does not exists
*/

    public static DisjunctiveLowerBound findLast(ApplicationDataset base){
        int size=base.getListDisjunctiveLowerBound().size();
        if (size > 0) {
             return base.getListDisjunctiveLowerBound().get(size-1);
        }
        return null;
    }
 ...
 * @param b DisjunctiveLowerBound compare this to that object
 * @return Boolean true if the objects match the same criteria
*/

    public Boolean applicationSame(DisjunctiveLowerBound b){
...
*/
    ...
    public Boolean applicationEqual(DisjunctiveLowerBound b){
...
*/
    ...
    public void check(){
        if (getApplicationDataset() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"applicationDataset","DisjunctiveLowerBound",(getApplicationDataset()==null?"null":getApplicationDataset().toString()),"",WarningType.NOTNULL);
        }
        if (getDisjunctiveResource() == null){
         new ApplicationWarning(getApplicationDataset(),ApplicationDataset.getIdNr(),toColumnString(),"disjunctiveResource","DisjunctiveLowerBound",(getDisjunctiveResource()==null?"null":getDisjunctiveResource().toString()),"",WarningType.NOTNULL);
        }
    }

    static void dummy(ApplicationDataset base){
// no dummy information for class DisjunctiveLowerBound
    }
