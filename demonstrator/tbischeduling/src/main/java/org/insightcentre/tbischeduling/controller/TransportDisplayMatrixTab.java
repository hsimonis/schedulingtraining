...
public class TransportDisplayMatrixTab extends MatrixTab {
	...
	protected TableColumn<ObservableList<ObjectProperty<ApplicationObject>>, String> createColumn(
		...
		col.setCellValueFactory(features -> {
			...
			String label = makeLabel(obj, TransportMatrix.class, "getValue");
			return new SimpleStringProperty(label);
		});
		col.setOnEditCommit(event -> {
			TransportMatrix bw = (TransportMatrix) table.getSelectionModel().getSelectedItem().get(index).getValue();
