...
None;
private static DurationDisplay[] cache = null;

public static DurationDisplay[] cachedValues(){
    if (cache== null){
        cache = DurationDisplay.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[DurationDisplay.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(DurationDisplay p:DurationDisplay.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[DurationDisplay.cachedValues().length];
    int i=0;
    for(DurationDisplay p:DurationDisplay.cachedValues()){
        res[i++] = p.name();
    }
...
public static DurationDisplay entry(int i){
...
public static int indexOf(DurationDisplay p){
    return p.ordinal();
}
