...
All;
private static LineChoice[] cache = null;

public static LineChoice[] cachedValues(){
    if (cache== null){
        cache = LineChoice.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[LineChoice.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(LineChoice p:LineChoice.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[LineChoice.cachedValues().length];
    int i=0;
    for(LineChoice p:LineChoice.cachedValues()){
        res[i++] = p.name();
    }
...
public static LineChoice entry(int i){
...
public static int indexOf(LineChoice p){
    return p.ordinal();
}
