...
public static ModelType[] cachedValues(){
    if (cache== null){
        cache = ModelType.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[ModelType.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(ModelType p:ModelType.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[ModelType.cachedValues().length];
    int i=0;
    for(ModelType p:ModelType.cachedValues()){
        res[i++] = p.name();
    }
...
public static ModelType entry(int i){
...
public static int indexOf(ModelType p){
    return p.ordinal();
}
