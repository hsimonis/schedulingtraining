...
NoWait;
private static NoWaitType[] cache = null;

public static NoWaitType[] cachedValues(){
    if (cache== null){
        cache = NoWaitType.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[NoWaitType.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(NoWaitType p:NoWaitType.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[NoWaitType.cachedValues().length];
    int i=0;
    for(NoWaitType p:NoWaitType.cachedValues()){
        res[i++] = p.name();
    }
...
public static NoWaitType entry(int i){
...
public static int indexOf(NoWaitType p){
    return p.ordinal();
}
