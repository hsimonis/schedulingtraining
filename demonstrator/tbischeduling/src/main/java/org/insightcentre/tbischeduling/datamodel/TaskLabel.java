...
None;
private static TaskLabel[] cache = null;

public static TaskLabel[] cachedValues(){
    if (cache== null){
        cache = TaskLabel.values();
    }
    return cache;
}

public static String[] getNamesAndAll(){
    String[] res = new String[TaskLabel.cachedValues().length+1];
    int i=0;
    res[i++] = "All";
    for(TaskLabel p:TaskLabel.cachedValues()){
        res[i++] = p.name();
    }
    return res;
}
...
public static String[] getNames(){
    String[] res = new String[TaskLabel.cachedValues().length];
    int i=0;
    for(TaskLabel p:TaskLabel.cachedValues()){
        res[i++] = p.name();
    }
...
public static TaskLabel entry(int i){
...
public static int indexOf(TaskLabel p){
    return p.ordinal();
}
