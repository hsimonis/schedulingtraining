...
public class TransportMatrixController extends Table3Controller {
	...
	@FXML
	private TableColumn<TransportMatrix, String> name;

	@FXML
	private TableColumn<TransportMatrix, DisjunctiveResource> from;

	@FXML
	private TableColumn<TransportMatrix, DisjunctiveResource> to;

	@FXML
	private TableColumn<TransportMatrix, Integer> value;
	...
	public void setMainApp(AbstractJfxMainWindow app) {
		...
		table.setItems(mainApp.getTransportMatrixData());
		from.setCellFactory(ComboBoxTableCell.forTableColumn(mainApp.getDisjunctiveResourceData()));
		from.setOnEditCommit(event -> {table.getSelectionModel().getSelectedItem().setFrom(event.getNewValue()); mainApp.reset();});
		to.setCellFactory(ComboBoxTableCell.forTableColumn(mainApp.getDisjunctiveResourceData()));
		to.setOnEditCommit(event -> {table.getSelectionModel().getSelectedItem().setTo(event.getNewValue()); mainApp.reset();});
	}

	public TableView<TransportMatrix> getTable() {
		return table;
	}
	...
	public void filter(String attribute, String comparison, String text) {
		table.setItems(mainApp.getTransportMatrixData());
		try {
			ObservableList<TransportMatrix> filteredItems = FXCollections.observableArrayList();
			for (TransportMatrix item : table.getItems()) {
