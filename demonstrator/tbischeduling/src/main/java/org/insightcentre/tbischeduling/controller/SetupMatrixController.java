...
public class SetupMatrixController extends Table3Controller {
	...
	@FXML
	private TableColumn<SetupMatrix, String> name;

	@FXML
	private TableColumn<SetupMatrix, SetupType> from;

	@FXML
	private TableColumn<SetupMatrix, SetupType> to;

	@FXML
	private TableColumn<SetupMatrix, Integer> value;
	...
	public void setMainApp(AbstractJfxMainWindow app) {
		...
		to.setCellFactory(ComboBoxTableCell.forTableColumn(mainApp.getSetupTypeData()));
		to.setOnEditCommit(event -> {table.getSelectionModel().getSelectedItem().setTo(event.getNewValue()); mainApp.reset();});
	}

	public TableView<SetupMatrix> getTable() {
		return table;
	}
	...
	public void filter(String attribute, String comparison, String text) {
		table.setItems(mainApp.getSetupMatrixData());
		try {
			ObservableList<SetupMatrix> filteredItems = FXCollections.observableArrayList();
			for (SetupMatrix item : table.getItems()) {
